package nl.teamtwee.bep3.restaurant.inventory.core.domain;

import lombok.AccessLevel;
import lombok.Getter;
import lombok.Setter;
import nl.teamtwee.bep3.restaurant.inventory.core.domain.event.IngredientEvent;
import nl.teamtwee.bep3.restaurant.inventory.core.domain.exception.InsufficientStock;
    ...
    public void takeStock(int amount) {
        if (stock - amount < 0)
            throw new InsufficientStock(
                    String.format("Stock of Item: %s is: %s! Taking: %s results in: %s difference!",
                            id, stock, amount, stock - amount));
        this.stock -= amount;
    }
